          ...
          parseNoteIdByAlias: function (_callback) {
            ...
            Note.findOne({
              ...
              if (note) {
                ...
                // Fix: Take the filePath stored in the note model to gather the file
                ...
                var filePath = path.join(config.systemdocspath, noteId + '.md')
                if (Note.checkFileExist(filePath)) {
                  alert(noteId);
                  Note.create({
                    alias: noteId,
      ...
      beforeCreate: function (note, options, callback) {
        ...
        if (!note.content) {
          ...
          if (!note.alias) {
            ...
            filePath = path.join(config.systemdocspath, note.alias + '.md')
          }
          if (Note.checkFileExist(filePath)) {
