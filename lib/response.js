  ...
  // Check if you got the new one
    ...
    function actionPDF(req, res, note) {
      ...
      if (action === 'pdf-NextEvent') {
        options.marginTop = '5cm';
      } else if (action === 'pdf-nexbyte') {
        options.marginTop = '3.35cm';
      }

      if (extracted.html.includes('class="title"')) {
        logger.info('Found "class="title" in html, removing first Header')
        extracted.html = extracted.html.replace(/<h1.*\s{1}<div class="title"/gm, '<div class="title"')
      }
