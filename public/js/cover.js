...
const options = {
  valueNames: ['id', 'text', 'timestamp', 'fromNow', 'time', 'tags', 'pinned', 'title', 'namespace'],
  item: '<li class="col-xs-12 col-sm-6 col-md-6 col-lg-4">' +
            '<span class="id" style="display:none;"></span>' +
            '<a href="#">' +
                '<div class="item">' +
                    // TODO: Check why this works'<div class="ui-history-close fa fa-close fa-fw" data-toggle="modal" data-target=".delete-modal"></div>' +
                    '<div class="content">' +
                        '<h4 class="title"></h4>' +
                        '<p>' +
...
$('.md_file_save').click(() => {
  ...
  fileExists($('input.md_file_path').val(), (err, result) => {
    if (!err) {
      var filePath = result.filePath;
      // md_file_namespace
      if(result.fileExists){
        $('.file_danger_alert').removeClass('hide');
      }else{
        $('.file_danger_alert').addClass('hide');
        createFile($('input.md_file_path').val(), (err, result) => {
          if (!err) {
            document.location.href = '/newnotepath?namespace=' + $('.md_file_namespace').val() + '&filePath=' + filePath;
          }else{
            document.location.href = '/' + $('.md_file_namespace').val();
...
$('.ui-search').click(() => {
  document.location.href = '/open?' + $('#ui-search-input').val();
})
