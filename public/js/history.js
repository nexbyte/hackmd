...
export function getHistory (callback) {
  return getServerHistory(callback);
}
...
export function parseServerToHistory (list, callback) {
  console.log("parseServerToHistory");
  $.get(`${serverurl}/history`)
        .done(data => {
          console.log(data);
          if (data.history) {
...
export function parseStorageToHistory (list, callback) {
  console.log("parseStorageToHistory");
  if (store.enabled) {
    let data = store.get('notehistory')
    if (data) {
      if (typeof data === 'string') { data = JSON.parse(data) }
...
function parseToHistory (list, notehistory, callback) {
  if (!callback) return
    ...
    for (let i = 0; i < notehistory.length; i++) {
      ...
      const timestamp = moment(notehistory[i].updatedAt);

      notehistory[i].timestamp = timestamp.valueOf()
      notehistory[i].fromNow = timestamp.fromNow()
      notehistory[i].time = timestamp.format('llll')

      if(notehistory[i].namespace !== null){
        notehistory[i].id = notehistory[i].namespace
      }

      if(notehistory[i].filePath === null){
        notehistory[i].text = '!Not published doc'
      }else{
        notehistory[i].text = notehistory[i].filePath.split("/")[notehistory[i].filePath.split("/").length - 1]
      }
      //notehistory[i].text = S(notehistory[i].filePath).escapeHTML().s
