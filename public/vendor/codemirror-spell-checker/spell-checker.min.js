...
var Typo = function(e, t, r, n) {
    if (n = n || {}, this.platform = n.platform || "chrome", this.dictionary = null, this.rules = {}, this.dictionaryTable = {}, this.compoundRules = [], this.compoundRuleCodes = {}, this.replacementTable = [], this.flags = n.flags || {}, e) {
        if (this.dictionary = e, "chrome" == this.platform) t || (t = this._readFile(chrome.extension.getURL("lib/typo/dictionaries/" + e + "/" + e + ".aff"))), r || (r = this._readFile(chrome.extension.getURL("lib/typo/dictionaries/" + e + "/" + e + ".dic")));
        else {
        ...
        for (var a = 0, s = this.compoundRules.length; s > a; a++)
            for (var o = this.compoundRules[a], l = 0, u = o.length; u > l; l++) this.compoundRuleCodes[o[l]] = [];
        ...
        for (var a = 0, s = this.compoundRules.length; s > a; a++) {
            for (var h = this.compoundRules[a], c = "", l = 0, u = h.length; u > l; l++) {
                var p = h[l];
                c += p in this.compoundRuleCodes ? "(" + this.compoundRuleCodes[p].join("|") + ")" : p
            }
            this.compoundRules[a] = RegExp(c, "i")
        }
    }
    return this
};
Typo.prototype = {
    ...
    _parseAFF: function(e) {
        ...
        for (var r = e.split("\n"), n = 0, i = r.length; i > n; n++) {
            var a = r[n],
                ...
                o = s[0];
            if ("PFX" == o || "SFX" == o) {
                for (var l = s[1], u = s[2], h = parseInt(s[3], 10), c = [], p = n + 1, f = n + 1 + h; f > p; p++) {
                    var a = r[p],
                        d = a.split(/\s+/),
                        v = d[2],
                        g = d[3].split("/"),
                    ...
                    var y = this.parseRuleCodes(g[1]),
                        _ = d[4],
                        C = {};
                    C.add = m, y.length > 0 && (C.continuationClasses = y), "." !== _ && (C.match = RegExp("SFX" === o ? _ + "$" : "^" + _)), "0" != v && (C.remove = "SFX" === o ? RegExp(v + "$") : v), c.push(C)
                }
                t[l] = {
                    type: o,
                    combineable: "Y" == u,
                ...
                for (var h = parseInt(s[1], 10), p = n + 1, f = n + 1 + h; f > p; p++) {
                    var a = r[p],
                        d = a.split(/\s+/);
    ...
    _parseDIC: function(e) {
        ...
        for (var r = e.split("\n"), n = {}, i = 1, a = r.length; a > i; i++) {
            ...
            if (o.length > 1) {
                ...
                "NEEDAFFIX" in this.flags && -1 != u.indexOf(this.flags.NEEDAFFIX) || t(l, u);
                for (var h = 0, c = u.length; c > h; h++) {
                    var p = u[h],
                        f = this.rules[p];
                    if (f)
                        for (var d = this._applyRule(l, f), v = 0, g = d.length; g > v; v++) {
                            var m = d[v];
                            if (t(m, []), f.combineable)
                                for (var y = h + 1; c > y; y++) {
                                    ...
                                    if (C && C.combineable && f.type != C.type)
                                        for (var R = this._applyRule(m, C), b = 0, F = R.length; F > b; b++) {
                                            var x = R[b];
                                            t(x, [])
                                        }
                                }
    ...
    _applyRule: function(e, t) {
        for (var r = t.entries, n = [], i = 0, a = r.length; a > i; i++) {
            ...
            if (!s.match || e.match(s.match)) {
                var o = e;
                if (s.remove && (o = o.replace(s.remove, "")), "SFX" === t.type ? o += s.add : o = s.add + o, n.push(o), "continuationClasses" in s)
    ...
    hasFlag: function(e, t, r) {
        if (t in this.flags) {
            if (void 0 === r) var r = Array.prototype.concat.apply([], this.dictionaryTable[e]);
    ...
    suggest: function(e, t) {
        function r(e) {
            for (var t = [], r = 0, n = e.length; n > r; r++) {
                for (var i = e[r], a = [], s = 0, o = i.length + 1; o > s; s++) a.push([i.substring(0, s), i.substring(s, i.length)]);
                ...
                for (var c = [], s = 0, o = a.length; o > s; s++) {
                    var h = a[s];
                    h[1].length > 1 && c.push(h[0] + h[1][1] + h[1][0] + h[1].substring(2))
                }
                for (var p = [], s = 0, o = a.length; o > s; s++) {
                    var h = a[s];
                    if (h[1])
                        for (var f = 0, d = u.alphabet.length; d > f; f++) p.push(h[0] + u.alphabet[f] + h[1].substring(1))
                }
                for (var v = [], s = 0, o = a.length; o > s; s++) {
                    var h = a[s];
                    if (h[1])
                        for (var f = 0, d = u.alphabet.length; d > f; f++) p.push(h[0] + u.alphabet[f] + h[1])
                }
                t = t.concat(l), t = t.concat(c), t = t.concat(p), t = t.concat(v)
            }
            return t
        }
        ...
        function i(e) {
            ...
            for (var a = r([e]), s = r(a), o = n(a).concat(n(s)), l = {}, h = 0, c = o.length; c > h; h++) o[h] in l ? l[o[h]] += 1 : l[o[h]] = 1;
            var p = [];
            for (var h in l) p.push([h, l[h]]);
            p.sort(i).reverse();
            for (var f = [], h = 0, c = Math.min(t, p.length); c > h; h++) u.hasFlag(p[h][0], "NOSUGGEST") || f.push(p[h][0]);
            return f
        }
...
var aff_loading = !1,
    ...
    dic_data = "",
    typo;
CodeMirror.defineMode("spell-checker", function(e) {
    if (!aff_loading) {
        aff_loading = !0;
        var t = new XMLHttpRequest;
        t.open("GET", serverurl + "/vendor/codemirror-spell-checker/de_CH.aff", !0), t.onload = function() {
            4 === t.readyState && 200 === t.status && (aff_data = t.responseText, num_loaded++, 2 == num_loaded && (typo = new Typo("de_CH", aff_data, dic_data, {
                platform: "any"
            })))
        }, t.send(null)
    }
    if (!dic_loading) {
        dic_loading = !0;
        var r = new XMLHttpRequest;
        r.open("GET", serverurl + "/vendor/codemirror-spell-checker/de_CH.dic", !0), r.onload = function() {
            4 === r.readyState && 200 === r.status && (dic_data = r.responseText, num_loaded++, 2 == num_loaded && (typo = new Typo("de_CH", aff_data, dic_data, {
                platform: "any"
            })))
        }, r.send(null)
    }
    var n = "!\"#$%&()*+,-./:;<=>?@[\\]^_`{|}~ ",
        ...
        a = {
            token: function(e) {
                var t = e.peek(),
                    r = "";
                if (n.includes(t)) return e.next(), null;
                for (; null != (t = e.peek()) && !n.includes(t);) r += t, e.next();
                return i.test(r) && typo && !typo.check(r) ? "spell-error" : null
            }
        },
